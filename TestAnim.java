...
import reuo.resources.AnimationLoader;
...
public class TestAnim extends JFrame {
	public static void main(String[] args) {
		try {
			...
			TestAnim frame = new TestAnim(args[0], args[1]);
	...
	public TestAnim(String idxFile, String artFile) throws IOException {
		cache = new AnimationLoader(new RandomAccessFile(idxFile, "r")
				.getChannel(), new RandomAccessFile(artFile, "r").getChannel());
	...
	class DrawArea extends JComponent implements MouseListener {
		...
		void reload() {
			try {
				if(!cache.getEntry(index).isValid()) {
					return;
				}
				img = cache.get(index).frames[0].getBufferedImage();
				//img = ;
				/*
